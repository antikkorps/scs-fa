...
- Pré-requis : un **`forgejo-runner` (backend Docker) enregistré** ; les jobs tournent en conteneur → le service Postgres est joint par son nom `postgres`. Le label `ubuntu-latest` correspond au runner existant (celui qui sert aussi à Renovate). Les actions `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` sont résolues depuis github.com par le runner
