...
- [x] Workflow Gitea Actions (`.gitea/workflows/ci.yml`) sur **PR + push main** : service Postgres, pnpm@10 + Node 22, `pnpm install --frozen-lockfile`, **Biome ci**, **typecheck** (`pnpm -r typecheck`), création du schéma via **`drizzle-kit push --force`** (non-interactif ; `schema.ts` = source de vérité, pas de baseline de migration), puis **tests** (`pnpm -r test`)
...
- Pré-requis : un **`act_runner` Gitea enregistré** ; les jobs tournent en conteneur → le service Postgres est joint par son nom `postgres` (ajuster si le runner est en host-network). Les actions `actions/checkout`, `pnpm/action-setup`, `actions/setup-node` sont résolues depuis github.com par le runner
