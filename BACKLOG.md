...
**Story 11.9b** — Expédition : les points restés ouverts — 🔜 **À FAIRE** _(branche `feat/story-11.9b-shipping-followups` préparée le 2026-09-13, prévue le 2026-09-14)_

> Suite directe de la 11.9 (mergée, #79). À boucler avant la 11.8.

- **Retrait en armurerie** : une commande en `shipping_method = 'retrait'` resterait « Non expédiée » pour toujours, et le panneau admin proposerait quand même d'emballer des colis. ✅ **Vérifié le 2026-09-13** : **aucun code n'écrit `shipping_method`** — ni le tunnel (`panier`, `commande`), ni `POST /api/orders`. Le retrait **n'existe pas en pratique** : la colonne est là, le choix n'est proposé nulle part. C'est donc d'abord une **décision produit** : proposer le retrait ou non ?
  - ⚠️ **Décision** : parcours dédié (« prête au retrait » → « retirée », sans colis ni transporteur) **vs** un « colis » sans transporteur. Faut-il tracer la **remise en main propre** d'une arme (date, qui a remis, contrôle d'identité) ? Un e-mail « votre commande vous attend » ?
- **Livraison saisie à la main** : aucun suivi transporteur automatique.
  - ⚠️ **Décision** : rester manuel (acceptable au lancement, volume faible) **vs** interroger une API de suivi (La Poste / Colissimo, agrégateur type AfterShip) — dépendance, clés et coût à peser.
- **Avertissements d'hydratation sur tout l'admin** (vus au smoke 11.9, **préexistants**, `/admin/produits` compris) : les pages en `useAsyncData({ server: false })` sont rendues par le serveur dans leur état vide ou d'erreur (« Aucune commande pour ces critères », « Commande introuvable ») pendant que le client attend « Chargement… ». Pistes : `routeRules` `{ ssr: false }` sur `/admin/**` (l'admin n'a aucun besoin de SSR ni de SEO), ou rendre l'état `pending` dès le serveur.
- **E-mails en dev** : l'`.env` API de dev pointe sur le **vrai relais OVH** — un smoke qui expédie un colis envoie un vrai e-mail. ⚠️ **Décision infra** : piège SMTP local par défaut en dev (ex. Mailpit dans le docker-compose) ?
- **Done** : tests-first, `pnpm -r typecheck` + Biome clean, smoke réel (commande en retrait menée jusqu'à la remise ; plus aucun avertissement d'hydratation sur l'admin).
